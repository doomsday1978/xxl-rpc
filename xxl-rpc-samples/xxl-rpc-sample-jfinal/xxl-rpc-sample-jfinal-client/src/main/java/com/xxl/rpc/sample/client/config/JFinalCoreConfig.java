...
public class JFinalCoreConfig extends JFinalConfig {
	...
	private void initXxlRpcClient() throws Exception {
		...
		xxlRpcInvokerFactory = new XxlRpcInvokerFactory(XxlRegistryServiceRegistry.class,
				new HashMap<String, String>() {
					{
						put(XxlRegistryServiceRegistry.XXL_REGISTRY_ADDRESS,
								xxlJobProp.get("xxl-rpc.registry.xxlregistry.address"));
		...
		if (beanFields != null && beanFields.length > 0) {
			...
			for (Field field : beanFields) {
				...
				// init reference bean
				XxlRpcReferenceBean referenceBean = new XxlRpcReferenceBean(rpcReference.netType(),
						rpcReference.serializer().getSerializer(), rpcReference.callType(), rpcReference.loadBalance(),
						iface, rpcReference.version(), rpcReference.timeout(), rpcReference.address(),
						rpcReference.accessToken(), null, xxlRpcInvokerFactory);
				...
				logger.info(
						...
						XxlRpcProviderFactory.makeServiceKey(iface.getName(), rpcReference.version()),
