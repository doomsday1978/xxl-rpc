...
public class XxlRpcClientAplication {
	public static void main(String[] args) throws Exception {
		 ...
		 */
		/*
		 * String serviceKey = XxlRpcProviderFactory.makeServiceKey(DemoService.class.getName(), null); // registry
		 * local
		 * invokerFactory.getServiceRegistry().registry(new HashSet<String>(Arrays.asList(serviceKey)),
		 * "127.0.0.1:7080");
		 */
		// test
	...
	public static void testSYNC() {
		// init client
		DemoService demoService = (DemoService) new XxlRpcReferenceBean(NetEnum.NETTY,
				Serializer.SerializeEnum.HESSIAN.getSerializer(), CallType.SYNC, LoadBalance.ROUND, DemoService.class,
				null, 500, "127.0.0.1:7080", null, null, null).getObject();
		// test
	...
	public static void testFUTURE() throws ExecutionException, InterruptedException {
		// client test
		DemoService demoService = (DemoService) new XxlRpcReferenceBean(NetEnum.NETTY,
				Serializer.SerializeEnum.HESSIAN.getSerializer(), CallType.FUTURE, LoadBalance.ROUND, DemoService.class,
				null, 500, "127.0.0.1:7080", null, null, null).getObject();
		// test
	...
	public static void testCALLBACK() throws ExecutionException, InterruptedException {
		// client test
		DemoService demoService = (DemoService) new XxlRpcReferenceBean(NetEnum.NETTY,
				Serializer.SerializeEnum.HESSIAN.getSerializer(), CallType.CALLBACK, LoadBalance.ROUND,
	...
	public static void testONEWAY() throws ExecutionException, InterruptedException {
		// client test
		DemoService demoService = (DemoService) new XxlRpcReferenceBean(NetEnum.NETTY,
				Serializer.SerializeEnum.HESSIAN.getSerializer(), CallType.ONEWAY, LoadBalance.ROUND, DemoService.class,
				null, 500, "127.0.0.1:7080", null, null, null).getObject();
		// test
