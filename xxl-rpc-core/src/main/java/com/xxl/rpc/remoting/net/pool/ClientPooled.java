...
public abstract class ClientPooled {
	...
	// ---------------------- iface ----------------------
	public abstract void init(String host, int port, final Serializer serializer,
	...
	// ---------------------- client pool map ----------------------
	private static ConcurrentHashMap<String, GenericObjectPool<ClientPooled>> clientPoolMap; // (static) alread
																								// addStopCallBack

	public static GenericObjectPool<ClientPooled> getPool(String address, Class<? extends ClientPooled> clientPoolImpl,
			Serializer serializer, final XxlRpcInvokerFactory xxlRpcInvokerFactory) throws Exception {
		...
		synchronized (clientPoolMap) {
			...
			// set pool
			clientPool = new GenericObjectPool<ClientPooled>(
					new ClientPoolFactory(clientPoolImpl, host, port, serializer, xxlRpcInvokerFactory));
